...
class InputFeatures(object):
  ...
  def __eq__(self, other):
    return (self.input_ids == other.input_ids and
            ...
            self.is_real_example == other.is_real_example and
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  #print('example.guid',(example.guid).replace('eval-',''))
  feature = InputFeatures(
      ...
      label_id=label_id,
      is_real_example=True,
      guid=(example.guid).replace('eval-',''))
  return feature
...
def main(_):
  ...
  #result = estimator.evaluate(train_input_fn, steps=10)
  ...
  if FLAGS.do_predict:
    ...
    features = convert_examples_to_features(examples=predict_examples,label_list=label_list, 
    ...
    for feature in features:
        ...
        #print('feature.guid',feature.guid)
        #print(feature.label_id)
        unique_id_to_feature[int(feature.guid)] = feature

    feature_label_id = []
    for feature in features:
        feature_label_id.append(feature.label_id)    
    
    #print(type(unique_id_to_feature))
    #print(len(unique_id_to_feature))
    #print('type(feature.guid)',type(feature.guid))
    #print('unique_id_to_feature',unique_id_to_feature[0])
    #exit()
    input_fn_try = input_fn_builder(
    ...
    count = 0
    for result in estimator.predict(input_fn_try):#yield_single_examples=True
        if count % 500 == 0:
            print('count',count)
        ...
        for layer in result:
            if layer == 'unique_ids':
                continue
            layer_output = result[layer]
            #print('layer_output[0].shape',layer_output[0].shape)
            embedding = np.add(embedding,layer_output[0])

        All_Embedding.append(embedding)
        if feature_label_id[count] == 0:
            All_Label.append('Devign Vulnerable')
        else:
            All_Label.append('Devign Non-Vulnerable')
        embedding = np.zeros(1024)
        count = count + 1
        print(All_Embedding)
    file = open('/notebooks/All_Embedding_Devign', 'wb')
    pickle.dump(All_Embedding, file)
    file.close()
    print(len(All_Embedding))
    
    file = open('/notebooks/All_Label_Devign', 'wb')
    pickle.dump(All_Label, file)
    file.close()
    print(len(All_Label))
    exit()
